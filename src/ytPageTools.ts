...
export class SpeedControlElems {
  rootPanel: HTMLDivElement;

  static createValid(node: Node): SpeedControlElems | null {
    if (node instanceof HTMLDivElement) {
      const result = new SpeedControlElems(node);
      if (result.isValid()) {
        return result;
      }
    }
  ...
  constructor(rootPanel: HTMLDivElement) {
    this.rootPanel = rootPanel;
  }

  isValid(): boolean {
    // class='ytp-panel ytp-panel-animate-forward'
    if (!this.rootPanel.classList.contains('ytp-panel')) {
      return false;
    }
    // 2 direct children
    if (this.rootPanel.children.length !== 2) { return false; }
    if (!(this.getPanelHeader() instanceof HTMLDivElement)) { return false; }
    if (!(this.getSPContent() instanceof HTMLDivElement)) { return false; }

    //ok
    return true;
  }
  getPanelHeader(): HTMLDivElement | null {
    return this.rootPanel.querySelector(':scope > div.ytp-panel-header');
  }
  getSPContent(): HTMLDivElement | null {
    return this.rootPanel.querySelector(':scope > div.ytp-variable-speed-panel-content');
  }
}
